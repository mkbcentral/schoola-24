<div>
    <div class="d-flex justify-content-end">
        <x-form.app-button wire:click="initForm" data-bs-toggle="modal" data-bs-target="#form-rate" textButton='Nouveau'
            ...
            @else
                @foreach ($rates as $index => $rate)
                    <tr wire:key='{{ $rate->id }}'>
                        ...
                        <td>
                            <span class="text-uppercase badge {{ $rate->is_changed ? 'bg-warning' : 'bg-success' }}">
                                {{ $rate->is_changed ? 'Changé' : "En cours d'utilisation" }}
                            </span>
                        </td>
                        <td class="">{{ $rate->created_at->format('d-m-Y') }}</td>
                                ...
                                <x-others.dropdown wire:ignore.self icon="bi bi-three-dots-vertical"
                                    ...
                                    @if ($rate->payments->isEmpty())
                                        <x-others.dropdown-link iconLink='bi bi-trash-fill' labelText='Supprimer'
                                            href="#" wire:confirm='Etês-vous sûr de supprimer ?'
    ...
    <x-modal.build-modal-fixed idModal='form-rate' size='md'
