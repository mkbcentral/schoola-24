    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-2">
                <div class="badge bg-primary mb-2 mb-md-0">
                    <h4 class="fw-medium mb-0">
                        TOTAL: {{ app_format_number($total_usd, 1) }} $ | {{ app_format_number($total_cdf, 1) }} Fc
                    </h4>
                </div>
                @can('view-payment')
                    <x-form.app-button wire:click='newOtherExpenseFee' data-bs-toggle="modal"
                        data-bs-target="#form-other-expense-fee" textButton='Nouveau' icon="bi bi-plus-circle"
            ...
            <div class="row mt-3 g-2">
                ...
                <div class="col-12 col-md-auto">
                    <div class="d-flex align-items-center">
                        ...
                        <select id="my-select" class="form-control form-select" wire:model.live='currency_filter'>
                    ...
                    @else
                        <tbody>
                            @foreach ($otherExpenses as $index => $otherExpense)
                                <tr wire:key='{{ $otherExpense->id }}'>
                                    <td class="text-center">{{ $index + 1 }}</td>
                                    <td>{{ $otherExpense->created_at->format('d/m/Y') }}</td>
                                    <td>{{ $otherExpense->description }}</td>
                                    <td>{{ $otherExpense->otherSourceExpense->name }}</td>
                                    <td>{{ $otherExpense->categoryExpense->name }}</td>
                                    <td>{{ format_fr_month_name($otherExpense->month) }}</td>
                                    <td class="text-end">
                                        ...
                                        @can('manage-payment')
                                            <x-others.dropdown wire:ignore.self icon="bi bi-three-dots-vertical"
                                                ...
                                                <x-others.dropdown-link iconLink='bi bi-pencil-fill' labelText='Editer'
                                                    ...
                                                    data-bs-target="#form-other-expense-fee"
