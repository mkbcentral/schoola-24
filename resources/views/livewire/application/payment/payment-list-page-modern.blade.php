...
<div class="space-y-4">
    ...
    <x-content.main-content-page>
                        ...
                        @foreach ($totalsByCurrency as $currency => $amount)
                            <div class="bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-3 py-1 rounded-md font-medium text-sm">
                                        {{ $currency }}
                                    </div>
                                </div>
                            </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4">
                    ...
                    <div class="lg:col-span-2 flex items-end">
                        <button class="w-full px-4 py-2.5 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 font-medium rounded-lg transition-colors flex items-center justify-center gap-2" 
                                type="button"
        ...
        <x-offcanvas-filters title="Filtres supplémentaires" icon="bi-funnel-fill">
            ...
            <x-slot name="actions">
                <div class="space-y-2">
                    <button class="w-full px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2" 
                            wire:click="resetFilters"
                            @click="$dispatch('close-filters')">
                        <i class="bi bi-arrow-clockwise"></i> Réinitialiser tous les filtres
                    </button>
                    <button class="w-full px-4 py-2.5 bg-gray-500 hover:bg-gray-600 text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2" 
                            @click="$dispatch('close-filters')">
            ...
            <div class="bg-gray-100 dark:bg-gray-700 px-6 py-3 border-b border-gray-200 dark:border-gray-600">
                <div class="flex justify-between items-center">
                    <h3 class="text-gray-700 dark:text-gray-200 font-semibold text-base flex items-center gap-2">
                        ...
                        Liste des paiements
                        <span class="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-1 rounded-md text-sm font-medium ml-2">
                            {{ $totalCount }}
                        </span>
                    </h3>
                </div>
            ...
            <div class="p-4 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    @if ($categoryFeeId && isset($categoryFees))
                        ...
                        @endphp
                        @if ($selectedCategory)
                            <div class="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium">
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                        <tr class="border-b border-gray-200 dark:border-gray-600">
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">#</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">N° Paiement</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Élève</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Catégorie</th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Montant</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Mois</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Statut</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wide">Date</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($payments as $payment)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <span class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-1 rounded-md text-xs font-medium">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span class="bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 px-2 py-1 rounded-md text-xs font-medium">
                                        {{ format_fr_month_name($payment->month) ?? '-' }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    @if ($payment->is_paid)
                                        <span class="inline-flex items-center gap-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-1 rounded-full text-xs font-medium border border-green-300 dark:border-green-700">
                                            <i class="bi bi-check-circle-fill"></i> Payé
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 px-2 py-1 rounded-full text-xs font-medium border border-red-300 dark:border-red-700">
                                            <i class="bi bi-x-circle-fill"></i> Non payé
                                        </span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <small class="text-gray-500 dark:text-gray-400 text-xs flex items-center justify-center gap-1">
