    ...
    <div class="row mb-4">
        <div class="col-md-3">
            <x-widget.card-info data-bs-toggle="modal" data-bs-target="#form-cost-recipe" label="RECETTES MINERVAL"
                icon="bi-cash-coin" value="{{ app_format_number($revenue, 1) }}$" linkLabel="Voir détail" bg="bg-info" />
        </div>
        <div class="col-md-3">
            <x-widget.card-info data-bs-toggle="modal" data-bs-target="#dialog-cost-expense"
                label="Depense sur le frais" icon="bi-wallet2" :value="app_format_number($expense->total_usd, 1)" link="" linkLabel="Voir détail"
                bg="bg-danger" />
        </div>
