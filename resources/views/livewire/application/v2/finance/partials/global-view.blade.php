...
<div class="card mb-4 border-0" style="box-shadow: 0 2px 12px rgba(0, 0, 0, 0.08);">
    <div class="card-body p-4">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h6 class="mb-2 fw-semibold">Filtrer par catégorie de frais</h6>
                @if (!empty($categories))
                    <select wire:model.live="category_fee_id_filter" class="form-select" style="max-width: 300px;">
                        <option value="">Toutes les catégories</option>
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}">{{ $category->name }}</option>
                        @endforeach
                    </select>
                    @if (!empty($category_fee_id_filter))
                        <small class="text-muted mt-2 d-block">
                            <i class="bi bi-info-circle me-1"></i>
                            Filtrage actif par catégorie
                        </small>
                    @endif
                @endif
            </div>
