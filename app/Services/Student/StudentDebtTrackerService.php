...
class StudentDebtTrackerService
{
    ...
    public function canPayForMonth(int $registrationId, int $categoryFeeId, string $targetMonth): array
    {
        ...
        foreach ($monthsOrder as $moisLabel => $moisNum) {
            if ($moisNum == $targetNum) {
                break;
            }
            // Si l'élève n'était pas inscrit à ce mois, on ne bloque pas
            if ($inscriptionDate) {
                // Comparer la date complète (année + mois calendaire)
                // Créer une date hypothétique pour le mois à vérifier
                $monthToCheckDate = \Carbon\Carbon::createFromDate($inscriptionDate->year, $moisNum, 1);

                // Si le mois à vérifier est avant la date d'inscription, ignorer
                if ($monthToCheckDate < $inscriptionDate) {
                    continue;
                }
            }
